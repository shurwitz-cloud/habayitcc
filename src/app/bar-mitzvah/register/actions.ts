...
function paymentPlanLabel(plan: BmxPaymentPlan): string {
  switch (plan) {
    case 'full':
...
export async function submitBmxRegistration(
  input: RegistrationInput
): Promise<RegistrationResult> {
  const limited = await enforceActionRateLimit('bmx-register', 8, 15 * 60 * 1000);
  ...
  try {
    ...
    if (familyError || !family) {
      console.error('[bmx registration] family insert error:', familyError);
    ...
    if (parentsError) {
      console.error('[bmx registration] parents insert error:', parentsError);
    ...
    if (!programId) {
      ...
      if (programError || !createdProgram?.id) {
        console.error('[bmx registration] could not create program:', programError);
        return {
          success: false,
    ...
    for (let i = 0; i < input.children.length; i++) {
      ...
      if (childError || !childRowData) {
        console.error('[bmx registration] child insert error:', childError, child);
        return {
          success: false,
      ...
      if (regError) {
        console.error('[bmx registration] program_registrations insert error:', regError);
        return {
          success: false,
    ...
    const { error: waiverError } = await supabase.from('waivers').insert({
      family_id: family.id,
      waiver_type: 'bmx_policies',
      signed_by: `${input.parent1FirstName} ${input.parent1LastName}`.trim(),
      document_version: '2026-v1',
    });
    if (waiverError) {
      console.error('[bmx registration] waiver insert error:', waiverError);
    }

    void bmxRegistrationRow({
    ...
    await sendRegistrationReceivedEmail({
      ...
      programName: BMX_NAME,
      programPath: BMX_PATH,
    });

    return { success: true };
  } catch (err) {
    console.error('HaBayit BMX registration error:', err);
