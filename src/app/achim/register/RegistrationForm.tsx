...
import { submitAchimRegistration, type ChildInput, type RegistrationInput } from './actions';
...
const PAYMENT_PLAN_OPTIONS: {
...
}[] = [
  {
    value: 'full',
    title: 'Pay in full',
    description: `Charged upon acceptance — $${ACHIM_PAY_IN_FULL_DISCOUNT} off.`,
  },
  {
...
export function RegistrationForm() {
  ...
  const [paymentPlan, setPaymentPlan] = useState<AchimPaymentPlan>('full');
  const [paymentMethod, setPaymentMethod] = useState<AchimPaymentMethod | ''>('');
  ...
  function calculateTuitionSubtotal() {
    return getAchimSessionTuition(isChaiPartner, paymentPlan);
  }

  function calculateTotal() {
    if (!paymentMethod) return calculateTuitionSubtotal();
    return getAchimGrandTotal(calculateTuitionSubtotal(), paymentMethod);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await submitAchimRegistration(input);
    setSubmitting(false);
    ...
    return (
      <div className="max-w-[600px] mx-auto text-center py-20">
        ...
        <p className="text-muted">
          Your {ACHIM_NAME} registration has been submitted. Your payment method is on
      ...
      <FormSection title="Tuition & Payment">
        <div className="grid md:grid-cols-2 gap-4.5 mb-5">
          <div className="bg-soft border border-line rounded-[18px] p-5.5">
            ...
            <div className="text-muted text-[0.85rem] mt-2">
              Every Tuesday · {ACHIM_SESSION_MONTHS}-month program (Sep–May)
            </div>
          </div>
        ...
        <div className="mt-5">
          <Field label="Payment plan" required>
            <select
              value={paymentPlan}
              onChange={(e) => setPaymentPlan(e.target.value as AchimPaymentPlan)}
              required
            >
        ...
        <div className="bg-soft border border-line rounded-[18px] p-5 mt-5">
          ...
          {paymentPlan === 'full' && (
            <div className="flex justify-between py-2 border-b border-black/[0.06] text-muted">
              ...
              <strong className="tabular-nums">-${ACHIM_PAY_IN_FULL_DISCOUNT}</strong>
            </div>
          )}
          {paymentMethod === 'card' && (
            <div className="flex justify-between py-2 border-b border-black/[0.06] text-muted">
              <span>Card processing fee ({ACHIM_CARD_PROCESSING_RATE * 100}%)</span>
              <strong className="tabular-nums">
                +${formatCurrency(getAchimCardProcessingFee(calculateTuitionSubtotal(), 'card'))}
              </strong>
            </div>
          )}
          {paymentMethod && (
      ...
      <FormSection title="Policies & Agreement">
        <div className="border border-line rounded-2xl p-4.5 bg-soft text-[0.9rem] space-y-3.5">
          ...
          <PolicyBlock title="Tuition & Payments">
            ...
            selected. A {ACHIM_CARD_PROCESSING_RATE * 100}% processing fee applies to
          ...
          <PolicyBlock title="Medical Emergencies">
            In the event of a medical emergency, {ACHIM_NAME} staff will make every
          ...
          <PolicyBlock title="Photography">
            ...
            or record your child during {ACHIM_NAME} and related activities. Children&apos;s
        ...
        <div className="mt-5">
          <label className="flex items-start gap-2.5 text-[0.95rem]">
            <input
              ...
              required
            />
            I have read and agree to the {ACHIM_NAME} Policies above.
          </label>
        </div>
