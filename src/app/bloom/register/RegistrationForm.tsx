...
import { submitBloomRegistration, type ChildInput, type RegistrationInput } from './actions';
...
const PAYMENT_PLAN_OPTIONS: {
...
}[] = [
  {
    value: 'full',
    title: 'Pay in full',
    description: `Charged upon acceptance — $${BLOOM_PAY_IN_FULL_DISCOUNT} off.`,
  },
  {
...
export function RegistrationForm() {
  ...
  const [paymentPlan, setPaymentPlan] = useState<BloomPaymentPlan>('full');
  const [paymentMethod, setPaymentMethod] = useState<BloomPaymentMethod | ''>('');
  ...
  function selectPaymentMethod(method: BloomPaymentMethod) {
  ...
  function calculateTuitionSubtotal() {
    return getBloomSessionTuition(isChaiPartner, paymentPlan);
  }

  function calculateTotal() {
    if (!paymentMethod) return calculateTuitionSubtotal();
    return getBloomGrandTotal(calculateTuitionSubtotal(), paymentMethod);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await submitBloomRegistration(input);
    setSubmitting(false);
    ...
    return (
      <div className="max-w-[600px] mx-auto text-center py-20">
        ...
        <p className="text-muted">
          Your {BLOOM_NAME} registration has been submitted. Your payment method is on
      ...
      {children.map((child, i) => (
        <div key={i} className="bg-white border-[1.5px] border-dashed border-line rounded-[18px] p-6">
          ...
          <div className="border-t border-line pt-5">
            ...
            <p className="text-muted text-[0.88rem] mb-4">This helps us place your daughter at the right level.</p>
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="mb-4">
                <Field label="Please specify" required>
                  <textarea
                    ...
                    placeholder="Share anything that will help us support your daughter."
                    rows={3}
                    required
                  />
                </Field>
      ...
      <FormSection title="Tuition & Payment">
        <div className="grid md:grid-cols-2 gap-4.5 mb-5">
          ...
          <div className="bg-soft border border-gold rounded-[18px] p-5.5">
            ...
            <div className="text-muted text-[0.85rem]">
              ${BLOOM_CHAI_DISCOUNT} for HaBayit Chai Partners with a valid code
            </div>
          </div>
        </div>
        ...
        {isBloomEarlyBirdActive() && (
          <div className="bg-soft border border-gold rounded-[18px] px-5 py-4 mb-5 text-[0.92rem] text-navy">
            <strong>Early registration:</strong> ${BLOOM_EARLY_BIRD_DISCOUNT} off when you sign up
            by {BLOOM_EARLY_BIRD_DEADLINE_LABEL}.
          </div>
        )}
          ...
          <label className="flex items-start gap-2.5">
            ...
            <span className="text-[1rem] font-bold text-navy">
              ...
              <span className="block text-muted text-[0.9rem] font-normal mt-0.5">
                HaBayit Chai Partners receive 1 month off (${BLOOM_CHAI_DISCOUNT}) with a valid
                code.{' '}
        ...
        <div className="mt-5">
          <Field label="Payment plan" required>
            <select
              value={paymentPlan}
              onChange={(e) => setPaymentPlan(e.target.value as BloomPaymentPlan)}
              required
            >
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <PaymentOption
            ...
            title="Credit Card"
            description={`${BLOOM_CARD_PROCESSING_RATE * 100}% processing fee added to tuition.`}
          />
        </div>
        ...
        <div className="bg-soft border border-line rounded-[18px] p-5 mt-5">
          ...
          <div className="flex justify-between py-2 border-b border-black/[0.06] text-muted">
            ...
            <strong className="tabular-nums">
              $
              {getBloomSessionTuition(isChaiPartner, 'two_installments', {
                earlyBird: false,
              }).toLocaleString()}
            </strong>
          </div>
          {isBloomEarlyBirdActive() && (
            <div className="flex justify-between py-2 border-b border-black/[0.06] text-muted">
              <span>Early registration (by {BLOOM_EARLY_BIRD_DEADLINE_LABEL})</span>
              <strong className="tabular-nums">-${BLOOM_EARLY_BIRD_DISCOUNT}</strong>
            </div>
          )}
          {paymentPlan === 'full' && (
            <div className="flex justify-between py-2 border-b border-black/[0.06] text-muted">
              <span>Pay-in-full discount</span>
              <strong className="tabular-nums">-${BLOOM_PAY_IN_FULL_DISCOUNT}</strong>
            </div>
          )}
          {paymentMethod === 'card' && (
            <div className="flex justify-between py-2 border-b border-black/[0.06] text-muted">
              <span>Card processing fee ({BLOOM_CARD_PROCESSING_RATE * 100}%)</span>
              <strong className="tabular-nums">
                +${formatCurrency(getBloomCardProcessingFee(calculateTuitionSubtotal(), 'card'))}
              </strong>
            </div>
          )}
          {paymentMethod && (
            ...
            <HebrewAdventurePaymentSetup
              ...
              returnUrl={
                typeof window !== 'undefined'
                  ? `${window.location.origin}/bloom/register?setup=complete`
                  : '/bloom/register?setup=complete'
              }
            />
      ...
      <FormSection title="Policies & Agreement">
        <div className="border border-line rounded-2xl p-4.5 bg-soft text-[0.9rem] space-y-3.5">
          ...
          <PolicyBlock title="Tuition & Payments">
            ...
            selected. A {BLOOM_CARD_PROCESSING_RATE * 100}% processing fee applies to
          ...
          <PolicyBlock title="Medical Emergencies">
            In the event of a medical emergency, {BLOOM_NAME} staff will make every
          ...
          <PolicyBlock title="Photography">
            ...
            or record your child during {BLOOM_NAME} and related activities. Children&apos;s
        ...
        <div className="mt-5">
          <label className="flex items-start gap-2.5 text-[0.95rem]">
            <input
              ...
              required
            />
            I have read and agree to the {BLOOM_NAME} Policies above.
          </label>
        </div>
