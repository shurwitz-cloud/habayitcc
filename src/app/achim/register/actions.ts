...
import { achimRegistrationRow } from '@/lib/google/sheets';
import { ACHIM_NAME, ACHIM_PATH, ACHIM_SLUG } from '@/lib/programs/names';
...
function paymentPlanLabel(plan: AchimPaymentPlan): string {
  switch (plan) {
    case 'full':
...
export async function submitAchimRegistration(
  input: RegistrationInput
): Promise<RegistrationResult> {
  const limited = await enforceActionRateLimit('achim-register', 8, 15 * 60 * 1000);
  ...
  try {
    ...
    if (familyError || !family) {
      console.error('[achim registration] family insert error:', familyError);
    ...
    if (parentsError) {
      console.error('[achim registration] parents insert error:', parentsError);
    ...
    for (let i = 0; i < input.children.length; i++) {
      ...
      if (childError || !childRowData) {
        console.error('[achim registration] child insert error:', childError, child);
        return {
          success: false,
      ...
      const tuitionTotal = getAchimSessionTuition(
        input.isChaiPartner,
      ...
      if (regError) {
        console.error('[achim registration] program_registrations insert error:', regError);
        return {
          success: false,
    ...
    const { error: waiverError } = await supabase.from('waivers').insert({
      family_id: family.id,
      waiver_type: 'achim_policies',
      signed_by: `${input.parent1FirstName} ${input.parent1LastName}`.trim(),
      document_version: '2026-v1',
    });
    if (waiverError) {
      console.error('[achim registration] waiver insert error:', waiverError);
    }

    void achimRegistrationRow({
    ...
    await sendRegistrationReceivedEmail({
      ...
      programName: ACHIM_NAME,
      programPath: ACHIM_PATH,
    });

    return { success: true };
  } catch (err) {
    console.error('HaBayit Achim registration error:', err);
